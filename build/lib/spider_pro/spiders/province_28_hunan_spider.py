...
class MySpider(Spider):
    ...
    def parse_categoy_urls(self, response):
        try:
            li_list = response.xpath('//ul[@class="wb-tree"]/li[2]/ul/li')
            for li in li_list:
    ...
    def parse_categoy_data_urls(self, response):
        try:
            li_list = response.xpath('//div[@class="tabview-hd"]/ul/li')[1:3]
            for li in li_list:
                ...
                if type_code in self.list_notice_category_num:
                    ...
                    notice = 0
                # print(type_url, response.meta.get('categoy'))
                yield scrapy.Request(url=type_url, callback=self.parse_all_urls,
                             meta={'categoy': response.meta.get('categoy'), 'notice': notice, 'type_code': type_code})
        except Exception as e:
            self.logger.error(f"发起数据请求失败 {e} {response.url=}")

    def parse_all_urls(self, response):
        try:
            if response.xpath('//ul[@class="ewb-page-items clearfix"]/li/span/text()'):
                if self.enable_incr:
                    page = 1
                    ...
                    pages = re.findall('/(\d+)', response.xpath('//ul[@class="ewb-page-items clearfix"]/li/span/text()')[-2].get())[0]  #总页数
                    ...
                    for num in range(1, int(pages) + 1):
                        ...
                        # print(response.meta.get('categoy'), data_all_url)
                        yield scrapy.Request(url=data_all_url, callback=self.parse_all_data,
                                             meta={'categoy': response.meta.get('categoy'), 'notice': response.meta['notice'],
                                                   'type_code': response.meta['type_code']})
                ...
                if self.enable_incr:
                    li_list = response.xpath('//ul[@class="wb-data-item"]/li')
                    for li in li_list:
                        pub_time = li.xpath('./span/text()').get() or ''
                        pub_time = get_accurate_pub_time(pub_time)
                        title_name = li.xpath('./div/a/@title').get()
                        if response.meta['type_code'] == '002002008':
                        ...
                        data_url = self.url + li.xpath('./div/a/@href').get() or ''
                        x, y, z = judge_dst_time_in_interval(pub_time, self.sdt_time, self.edt_time)
                        if x:
                            total = int(len(li_list))  # 总条数
                            self.logger.info(f"初始总数提取成功 {total=} {response.url=} {response.meta.get('proxy')}")
                            yield scrapy.Request(url=data_url, callback=self.parse_itme,
                                                 meta={'categoy': response.meta.get('categoy'), 'notice': response.meta['notice'],
                                                       'title_name': title_name, 'notice_type': notice_type, 'pub_time': pub_time})
                else:
                    li_list = response.xpath('//ul[@class="wb-data-item"]/li')
                    total = int(len(li_list))  # 总条数
                    self.logger.info(f"初始总数提取成功 {total=} {response.url=} {response.meta.get('proxy')}")
                    for li in li_list:
                        ...
                        if response.meta['type_code'] == '002002008':
                            ...
                            notice_type = response.meta['notice']
                        # print(response.meta.get('categoy'), title_name, data_url)
                        yield scrapy.Request(url=data_url, callback=self.parse_itme,
                                             meta={'categoy': response.meta.get('categoy'), 'notice': response.meta['notice'],
                                                   'title_name': title_name, 'notice_type': notice_type, 'pub_time': pub_time})
        ...
        try:
            ...
            for li in info_list:
                ...
                if response.meta['type_code'] == '002002008':
                    ...
                    notice_type = response.meta['notice']
                # print(info_url, title_name, pub_time)
                yield scrapy.Request(url=info_url, callback=self.parse_itme,
                                     meta={'categoy': response.meta.get('categoy'), 'notice_type': notice_type,
