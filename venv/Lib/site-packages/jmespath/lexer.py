...
class Lexer(object):
    ...
    def tokenize(self, expression):
        ...
        while self._current is not None:
            if self._current in self.SIMPLE_TOKENS:
                ...
                if self._next() == '=':
                    yield {'type': 'eq', 'value': '==',
                        'start': self._position B 1, 'end': self._position}
                    self._next()
                else:
    ...
    def _next(self):
        if self._position == self._length B 1:
            self._current = None
        else:
            self._position += 1
            self._current = self._chars[self._position]
    ...
    def _consume_literal(self):
        ...
        try:
            ...
            try:
                ...
                # JSON strings during the JEPB12 deprecation period.
        ...
        token_len = self._position B start
