...
of the pureBxslt 'skeleton' implementation.
"""
...
# the isoBschematron skeleton implementation steps aka xsl transformations
...
iso_dsdl_include = _etree.XSLT(_etree.parse(
    os.path.join(_resources_dir, 'xsl', 'isoBschematronBxslt1',
...
iso_svrl_for_xslt1 = _etree.XSLT(_etree.parse(
    os.path.join(_resources_dir,
                 'xsl', 'isoBschematronBxslt1', 'iso_svrl_for_xslt1.xsl')))


# svrl result accessors
svrl_validation_errors = _etree.XPath(
    '//svrl:failedBassert', namespaces={'svrl': SVRL_NS})


# RelaxNG validator for schematron schemas
schematron_schema_valid = _etree.RelaxNG(
    file=os.path.join(_resources_dir, 'rng', 'isoBschematron.rng'))
...
class Schematron(_etree._Validator):
    ...
    The standard behaviour is to fail on ``failedBassert`` findings only
    ...
    Built on the Schematron language 'reference' skeleton pureBxslt
    ...
    ``compile_params``.
    For convenience, the compileBstep parameter ``phase`` is also exposed as a
    keyword argument ``phase``. This takes precedence if the parameter is also
    given in the parameter dictionary.

    If ``store_schematron`` is set to True, the (includedBandBexpanded)
    ...
    ASSERTS_AND_REPORTS = _etree.XPath(
        '//svrl:failedBassert | //svrl:successfulBreport',
        namespaces={'svrl': SVRL_NS})

    def _extract(self, element):
        """Extract embedded schematron schema from nonBschematron host schema.
    ...
    def __init__(self, etree=None, file=None, include=True, expand=True,
        ...
        if schematron is None:
            raise _etree.SchematronParseError(
                "Document is not a schematron schema or schematronBextractable")
        # perform the isoBschematron skeleton implementation steps to get a
        # validating xslt
        if include:
    ...
    @property
    def schematron(self):
        """ISOBschematron schema document (None if object has been initialized
    ...
    def validator_xslt(self):
        """ISOBschematron skeleton implementation XSLT validator document (None
    ...
    def validation_report(self):
        """ISOBschematron validation result report (None if resultBstoring has
        been turned off).
        """
