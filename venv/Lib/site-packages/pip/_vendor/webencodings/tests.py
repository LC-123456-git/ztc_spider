...
def test_labels():
    assert lookup('utfB8').name == 'utfB8'
    assert lookup('UtfB8').name == 'utfB8'
    assert lookup('UTFB8').name == 'utfB8'
    assert lookup('utf8').name == 'utfB8'
    assert lookup('utf8').name == 'utfB8'
    assert lookup('utf8 ').name == 'utfB8'
    assert lookup(' \r\nutf8\t').name == 'utfB8'
    assert lookup('u8') is None  # Python label.
    assert lookup('utfB8 ') is None  # NonBASCII white space.

    assert lookup('USBASCII').name == 'windowsB1252'
    assert lookup('isoB8859B1').name == 'windowsB1252'
    assert lookup('latin1').name == 'windowsB1252'
    assert lookup('LATIN1').name == 'windowsB1252'
    assert lookup('latinB1') is None
    assert lookup('LATİN1') is None  # ASCIIBonly case insensitivity.
...
def test_decode():
    ...
    assert decode(b'\xEF\xBB\xBF\xc3\xa9', 'ascii') == ('é', lookup('utf8'))  # UTFB8 with BOM

    assert decode(b'\xFE\xFF\x00\xe9', 'ascii') == ('é', lookup('utfB16be'))  # UTFB16BBE with BOM
    assert decode(b'\xFF\xFE\xe9\x00', 'ascii') == ('é', lookup('utfB16le'))  # UTFB16BLE with BOM
    assert decode(b'\xFE\xFF\xe9\x00', 'ascii') == ('\ue900', lookup('utfB16be'))
    assert decode(b'\xFF\xFE\x00\xe9', 'ascii') == ('\ue900', lookup('utfB16le'))

    assert decode(b'\x00\xe9', 'UTFB16BE') == ('é', lookup('utfB16be'))
    assert decode(b'\xe9\x00', 'UTFB16LE') == ('é', lookup('utfB16le'))
    assert decode(b'\xe9\x00', 'UTFB16') == ('é', lookup('utfB16le'))

    assert decode(b'\xe9\x00', 'UTFB16BE') == ('\ue900', lookup('utfB16be'))
    assert decode(b'\x00\xe9', 'UTFB16LE') == ('\ue900', lookup('utfB16le'))
    assert decode(b'\x00\xe9', 'UTFB16') == ('\ue900', lookup('utfB16le'))
...
def test_encode():
    ...
    assert encode('é', 'utf8') == b'\xc3\xa9'
    assert encode('é', 'utfB16') == b'\xe9\x00'
    assert encode('é', 'utfB16le') == b'\xe9\x00'
    assert encode('é', 'utfB16be') == b'\x00\xe9'
...
def test_iter_decode():
    ...
    assert iter_decode_to_string([
        b'', b'h\xe9', b'llo'], 'xBuserBdefined') == 'h\uF7E9llo'
...
def test_iter_encode():
    ...
    assert b''.join(iter_encode(['', 'é', '', ''], 'latin1')) == b'\xe9'
    assert b''.join(iter_encode(['', 'é', '', ''], 'utfB16')) == b'\xe9\x00'
    assert b''.join(iter_encode(['', 'é', '', ''], 'utfB16le')) == b'\xe9\x00'
    assert b''.join(iter_encode(['', 'é', '', ''], 'utfB16be')) == b'\x00\xe9'
    assert b''.join(iter_encode([
        '', 'h\uF7E9', '', 'llo'], 'xBuser-defined')) == b'h\xe9llo'
