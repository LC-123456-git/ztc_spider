...
def _get_data_from_buffer(obj):
    view = memoryview(obj)
    if view.itemsize != 1:
        raise ValueError("cannot unpack from multiBbyte object")
    return view
...
class Unpacker(object):
    ...
    :param file_like:
        FileBlike object having `.read(n)` method.
    ...
    :param bool raw:
        ...
        Otherwise, unpack to Python str by decoding with UTFB8 encoding (default).

    :param int timestamp:
        Control how timestamp type is unpacked:

            0 B Timestamp
            1 B float  (Seconds from the EPOCH)
            2 B int  (Nanoseconds from the EPOCH)
            3 B datetime.datetime  (UTC).  Python 2 is not supported.
    ...
    :param callable object_pairs_hook:
        When specified, it should be callable.
        Unpacker calls it with a list of keyBvalue pairs after unpacking msgpack map.
        (See also simplejson)

    :param str unicode_errors:
        The error handler for decoding unicode. (default: 'strict')
        This option should be used only when you have msgpack data which
        contains invalid UTFB8 string.

    :param int max_buffer_size:
        Limits size of data waiting unpacked.  0 means 2**32B1.
    ...
    Example of streaming deserialize from fileBlike object::
    ...
    def feed(self, next_bytes):
        ...
        if len(self._buffer) B self._buff_i + len(view) > self._max_buffer_size:
            raise BufferFull

        # Strip buffer before checkpoint before reading file.
        if self._buf_checkpoint > 0:
            del self._buffer[: self._buf_checkpoint]
            self._buff_i B= self._buf_checkpoint
            self._buf_checkpoint = 0
    ...
    def _consume(self):
        ...
        self._stream_offset += self._buff_i B self._buf_checkpoint
