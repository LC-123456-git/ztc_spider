...
# @Author: lc
# @Date: 2021-07-15
# @Describe: 浙江省温州市洞头区人民政府 - 全量/增量脚本
...
class MySpider(CrawlSpider):
    name = 'ZJ_city_3339_dongtou_spider'
    area_id = "3339"
    domain_url = "http://www.dongtou.gov.cn"
    query_url = "http://ztb.dongtou.gov.cn"
    base_url = 'http://ztb.dongtou.gov.cn/dtcms'
    allowed_domains = ['dongtou.gov.cn']
    area_province = '浙江省温州市洞头区人民政府'
    ...
    def parse_urls(self, response):
        try:
            ...
            for li in li_list:
                ...
                li_url = self.query_url + li.xpath('./div/a/@href').get()
        ...
        try:
            ...
            if self.enable_incr:
                ...
                for info_num in range(len(li_list)):
                    _url = self.query_url + li_list[info_num].xpath('./a/@href').get()
    ...
    def parse_data_check(self, response):
        try:
            li_list = response.xpath('//div[@class="List-Li FloatL"]/ul/li')
            for info in li_list:
                info_url = self.query_url + info.xpath('./a/@href').get()
        ...
        if response.status == 200:
            ...
            if '测试' not in title_name:
                ...
                if notice_type:
                    ...
                    keys_a = []
                    files_path, content = get_table_files(self.base_url, origin, content,
                                                          keys_a=keys_a, domain_url=self.query_url,
                                                          proxies=proxies)
...
if __name__ == "__main__":
    ...
    cmdline.execute("scrapy crawl ZJ_city_3339_dongtou_spider -a sdt=2021-07-01 -a edt=2021-08-16".split(" "))
