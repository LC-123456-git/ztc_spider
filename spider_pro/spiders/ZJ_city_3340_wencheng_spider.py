...
# @Author: lc
# @Date: 2021-07-20
# @Describe: 浙江省温州市文成县人民政府 - 全量/增量脚本
...
from spider_pro.utils import judge_dst_time_in_interval, get_accurate_pub_time, \
     remove_specific_element, get_notice_type, get_table_files


class MySpider(CrawlSpider):
    name = 'ZJ_city_3340_wencheng_spider'
    area_id = "3340"
    domain_url = "http://ggjy.wencheng.gov.cn"
    query_url = "http://ggjy.wencheng.gov.cn/wccms"
    allowed_domains = ['ggjy.wencheng.gov.cn']
    area_province = '浙江省温州市文成县人民政府'

    # 招标预告
    list_tender_notice_num = ['招标文件预公示']
    # 招标公告
    list_notice_category_name = ['招标公告', '采购公告', '出让公告']
    # 招标变更
        ...
        try:
            ...
            for li in li_list:
                ...
                if notice:
                    ...
                    yield scrapy.Request(url=li_url, callback=self.parse_data_info, priority=100,
...
if __name__ == "__main__":
    ...
    # cmdline.execute("scrapy crawl ZJ_city_3340_wencheng_spider".split(" "))
    cmdline.execute("scrapy crawl ZJ_city_3340_wencheng_spider -a sdt=2021-05-01 -a edt=2021-07-19".split(" "))
