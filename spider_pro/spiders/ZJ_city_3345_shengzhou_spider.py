...
class MySpider(CrawlSpider):
    name = 'ZJ_city_3345_shengzhou_spider'
    area_id = "3345"
    domain_url = "http://www.szzj.gov.cn"
    query_url = "http://www.szzj.gov.cn/col/col1529991/index.html"
    base_url = 'http://www.szzj.gov.cn/module/jpage/dataproxy.jsp?startrecord={}&endrecord={}&perpage=40'
    allowed_domains = ['szzj.gov.cn']
    area_province = '浙江省绍兴嵊州市人民政府'

    # 招标预告
    list_tender_notice_num = []
    # 招标公告
    list_notice_category_name = ['招标公告', '招标文件公示']
    # 招标变更
    list_zb_abnormal_name = ['更正公告']
    # 中标预告
    list_win_advance_notice_name = ['中标候选人公示']
    # 中标公告
    list_win_notice_category_name = ['中标结果公告', '中标公示', '中标成交公示']
    # 招标异常
    list_alteration_category_name = []
    # 资格预审
    list_qualifiction_advance_num = []
    # 其他
    list_qita_num = ['合同信息公示']
    ...
    def parse_urls(self, response):
        try:
            li_list = response.xpath('//div[@class="gk_list"]/ul/li')[3:8]
            for li in li_list:
    ...
    def parse_data(self, response):
        try:
            li_list = response.xpath('//div[@class="gk_list"]/ul/li')[1:]
            for li in li_list:
                notice_url = li.xpath('./a/@href').get()
    ...
    def parse_item(self, response):
        if response.status == 200:
            ...
            if '测试' not in title_name:
                ...
                if notice_type:
                    content = response.xpath('//div[@id="ivs_content"]').get()
                    # 去除 title
                    _, content = remove_specific_element(content, 'h2', 'class', 'text-tag')
                    # 去除导航栏
                    _, content = remove_specific_element(content, 'table', 'bgcolor', '#F3F3F3')
                    # 去除 底部 下载 打印 关闭
                    _, content = remove_specific_element(content, 'td', 'height', '26')
                    # 去除 script
...
if __name__ == "__main__":
    ...
    # cmdline.execute("scrapy crawl ZJ_city_3345_shengzhou_spider".split(" "))
    cmdline.execute("scrapy crawl ZJ_city_3345_shengzhou_spider -a sdt=2021-07-01 -a edt=2021-08-19".split(" "))
