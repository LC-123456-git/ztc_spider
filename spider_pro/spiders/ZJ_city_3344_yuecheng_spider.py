...
# @Author: lc
# @Date: 2021-07-28
# @Describe: 浙江省绍兴市越城区人民政府 - 全量/增量脚本
...
class MySpider(CrawlSpider):
    name = 'ZJ_city_3344_yuecheng_spider'
    area_id = "3344"
    domain_url = "http://www.sxyc.gov.cn"
    query_url = "http://www.sxyc.gov.cn/col/col1559723/index.html"
    base_url = 'http://www.sxyc.gov.cn/module/jpage/dataproxy.jsp?startrecord={}&endrecord={}&perpage=40'
    allowed_domains = ['sxyc.gov.cn']
    area_province = '浙江省绍兴市越城区人民政府'

    # 招标预告
    list_tender_notice_num = ['意见征询']
    # 招标公告
    list_notice_category_name = ['招标公告', '招标文件公示', '采购公告', '国有集体产权交易信息', '拍卖代理机构招标']
    # 招标变更
    list_zb_abnormal_name = ['资格预审公示']
    # 中标预告
    list_win_advance_notice_name = []
    # 中标公告
    list_win_notice_category_name = ['结果公告', '成交公告', '交易结果公示', '中标公示']
    # 招标异常
    ...
    def parse_urls(self, response):
        try:
            li_list = response.xpath('//div[@class="width1200"]/div/div')[3:]
            for li in li_list:
                category = li.xpath('./div/span/text()').get()
                notice_list_url = li.xpath('./ul/li')[int(len(li.xpath('./ul/li'))/2):]
                notice_list_name = li.xpath('./ul/li')[:int(len(li.xpath('./ul/li'))/2)]
                for _url, _name in zip(notice_list_url, notice_list_name):
                    notice_url = _url.xpath('./a/@href').get()
                    notice_name = _name.xpath('./span/text()').get()
                    ...
                    if notice:
                        ...
                        yield scrapy.FormRequest(url=self.base_url.format(1, 120), callback=self.parse_data_info,
                                                 dont_filter=True, formdata=info_dict,
        ...
        if response.status == 200:
            ...
            if '测试' not in title_name:
                ...
                if notice_type:
                    ...
                    # 去除 title
                    _, content = remove_specific_element(content, 'p', 'class', 'con-title')
                    # 去除导航栏
                    _, content = remove_specific_element(content, 'div', 'class', 'small-title clearfix')
                    # 去除 底部 下载 打印 关闭
                    _, content = remove_specific_element(content, 'div', 'class', 'main-word clearfix')
